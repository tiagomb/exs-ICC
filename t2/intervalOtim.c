...
void potenciaIntervalo (intervalo_t intervalo, int expoente, intervalo_t *resultado){ //mudar pow
    if (expoente == 0){
        resultado->inicio = 1;
        resultado->fim = 1;
    } else if (expoente % 2 != 0 || intervalo.inicio >= 0){
        fesetround (FE_DOWNWARD);
        resultado->inicio = calculaPotencia(intervalo.inicio, expoente);
        fesetround (FE_UPWARD);
        resultado->fim = calculaPotencia(intervalo.fim, expoente);
    } else if (intervalo.fim < 0){
        fesetround (FE_DOWNWARD);
        resultado->inicio = calculaPotencia(intervalo.fim, expoente);
        fesetround (FE_UPWARD);
        resultado->fim = calculaPotencia(intervalo.inicio, expoente);
    } else{
        resultado->inicio = 0;
        resultado->fim = fmax (calculaPotencia(intervalo.inicio, expoente), calculaPotencia(intervalo.fim, expoente));
    }
}
